#!/usr/bin/env python
"""Create publication-quality summary visualizations for the T-SNE sweep (yaofeng_200M).
...
ax.set_title("yaofeng_200M: Composite Score vs. T-SNE Perplexity\n(green = passes verification)")
...
ax.set_ylabel("Composite Score")
ax.set_title("yaofeng_200M: Composite Score vs. HDBSCAN min_cluster_size\n(green = passes verification)")
ax.grid(axis="y", alpha=0.3)
ax.set_ylim(bottom=0)
...
ax.set_title("yaofeng_200M: Mean Composite Score\n(averaged over PCA dims & min_samples)")
...
records, _ = load_data(os.path.join(PROJECT_ROOT, "activation_data/yaofeng_200M/"),
                       subsample_rate=5)
activations, features, metadata = compute_features(records)

# Top 3 configs from the sweep (by composite score):
# 1. PCA=30, perp=30, mcs=400, ms=5   -> score=1.022
# 2. PCA=30, perp=30, mcs=400, ms=10  -> score=0.977
# 3. PCA=20, perp=50, mcs=50, ms=25   -> score=0.952
top_configs = [
    {"label": "#1: perp=30, mcs=400, ms=5", "perplexity": 30, "mcs": 400, "ms": 5},
    {"label": "#2: perp=30, mcs=400, ms=10", "perplexity": 30, "mcs": 400, "ms": 10},
    {"label": "#3: perp=50, mcs=50, ms=25", "perplexity": 50, "mcs": 50, "ms": 25},
]
...
fig.suptitle("yaofeng_200M: Top 3 T-SNE + HDBSCAN Configurations", fontsize=14, y=1.02)
fig.tight_layout()
