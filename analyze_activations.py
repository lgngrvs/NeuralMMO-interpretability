...
def load_data(path, subsample_rate=10):
    ...
    for json_file in json_files:
        ...
        label = f"Loading {json_file.name} ({file_size / 1024 / 1024:.0f} MB)"
        # File read is fast (OS cache); JSON parsing holds the GIL, so we
        # run it in a subprocess to keep the spinner thread alive.
        import concurrent.futures
        raw_text = json_file.read_text()
        with concurrent.futures.ProcessPoolExecutor(max_workers=1) as pool:
            future = pool.submit(json.loads, raw_text)
            with spinner(label):
...
def main():
    ...
    embedding = run_umap(activations, n_neighbors=args.umap_neighbors,
                         random_state=args.seed)
    labels = run_hdbscan(embedding, min_cluster_size=args.hdbscan_min_cluster,
